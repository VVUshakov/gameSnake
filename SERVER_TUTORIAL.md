...
Это пошаговое руководство для начинающих. Мы превратим обычную консольную игру «Змейка» в **сервер**, к которому смогут подключаться игроки с любых устройств (компьютер, телефон, планшет) через браузер или приложение.
...
### Что такое DTO
Когда сервер и клиент общаются, они не могут передавать объекты C# напрямую. Они отправляют **JSON-строки**. DTO — это классы, которые описывают, как данные будут выглядеть в JSON.
...
### Ключевая идея
`WebSocketRenderer` реализует тот же интерфейс `IGameRenderer`, что и старый `ConsoleRenderer`. Поэтому **ядро игры ничего не заметило** — оно вызывает `Render()`, как раньше.
...
### Почему очередь?
Клиент может нажать «вверх» три раза, пока сервер обрабатывает предыдущий кадр. Очередь сохраняет все команды. `ReadLoop` пишет в очередь, `ProcessInput` читает — и всё работает без конфликтов.
...
### Зачем
Интерфейс `IWindowConfigurator` требует кто-то реализовать. На сервере — делаем ничего. Это называется **Null Object Pattern**.
...
### Зачем
Каждый игрок — это отдельная «сессия». Менеджер хранит всех подключённых игроков и считает статистику.
...
### Проблема
`GameLoop.Update()` был `private`. Наша сессия вызывает его напрямую (для гибкости).
...
### Зачем
Сессия может захотеть обновить игру вручную, а не через `Game.Run()`. Публичный метод даёт гибкость.
